...
function convertMinuteToTime(time) {
  const hour = Math.floor(time / 60)
  const min = time % 60

  return (
    "0".repeat(2 - hour.toString().length) +
    `${hour}:` +
    "0".repeat(2 - min.toString().length) +
    `${min}`
  )
}
...
function solution(n, t, m, timetable) {
  ...
  for (let bus = 0; bus < n; bus += 1) {
    ...
    if (bus === n - 1) {
      ...
      // 이전 버스들은 보내도 된다.
      if (possiblePeople <= m) {
        // 현재 탈 수 있는 사람이 m명보다 작거나 같으면 현재 탈 수 있는 사람은 다 탈 수 있다.
        timeTable.splice(0, possiblePeople)
      } else {
        // 현재 탈 수 있는 사람이 m명보다 크면 m명만 갈 수 있다
  ...
  answer = convertMinuteToTime(startTime)
